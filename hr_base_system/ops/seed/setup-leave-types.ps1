...
Write-Host "" 
Write-Host "📋 Checking existing leave types..." -ForegroundColor Cyan
...
Write-Host ""
Write-Host "🚀 Creating leave types..." -ForegroundColor Cyan
...
foreach ($leaveType in $leaveTypes) {
    ...
    try {
        $null = Invoke-RestMethod -Uri "$BackendUrl/leave/types" -Method POST -Body $body -Headers $headers
...
try {
    ...
    Write-Host ""
    Write-Host "📊 Current leave types:" -ForegroundColor Cyan
...
Write-Host ""
Write-Host "✨ Leave types setup complete!" -ForegroundColor Green
